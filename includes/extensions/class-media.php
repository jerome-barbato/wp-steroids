...
class WPS_Media {
    ...
    public function wpmuOptions()
    {
        ...
        if( count($thumbnails) )
        {
            echo '<tr>
				...
				<td><a class="button button-primary" href="'.get_admin_url().'?clear_all_thumbnails">Remove '.count($thumbnails).' image(s)</a></td>
			</tr>';
        }
    ...
    function addCropField($form_fields, $post ) {
        ...
        foreach (['center', 'top', 'bottom', 'left', 'right', 'top-center', 'bottom-center'] as $option)
            $select .= '<option value="'.$option.'" '.($field_value==$option?'selected':'').'>'.__t(ucfirst(str_replace('-', ' ', $option))).'</option>';
